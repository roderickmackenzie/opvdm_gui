#!/usr/bin/env python
#    Organic Photovoltaic Device Model - a drift diffusion base organic solar cell model
...
class NotebookExample:
	...
	def notebook_load_pages(self):
		...
		if (os.path.exists("sim.opvdm")==True) and (os.getcwd()!="C:\\opvdm"):
			...
			internal_names = ["Device","JV Curve","JV simple","Light","Output","CELIV","Numerics", "ToF", "stark", "Bands", "Pulse","Pulse voc", "imps","Exp. Optical Model","Terminal","Sun voc","TPC","fit"]
			internal_files = ["device.inp","jv.inp","jv_simple.inp","light.inp","dump.inp","celiv.inp","math.inp","tof.inp","stark.inp","lumo0.inp","server.inp","pulse_voc.inp","imps.inp","light_exp.inp","terminal.inp","sun_voc.inp","tpc.inp","fit.inp"]
